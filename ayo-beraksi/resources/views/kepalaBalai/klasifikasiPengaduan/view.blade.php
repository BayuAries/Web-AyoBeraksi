...
@section('script')
<script>
    var showurl = '{{ route('klasifikasi.pengaduan.showMonitorPelaporan', '_rowid') }}';
    // var editurl = '{{ route('klasifikasi.pengaduan.edit', '_rowid') }}';
    $(document).ready(function() {
        // Databale for List
        var t = $('#datable').DataTable({
            ajax:{
                url: '{{ route('actions.klasifikasi.pengaduan.list') }}',
                dataSrc: 'data'
            },
            columns: [
                {data: 'id',
                ...
                {data: 'id_pengaduan.nama_pelapor'},
                {data: 'id_pengaduan.nik'},
                {data: 'id_pengaduan.tanggal_pengaduan',
